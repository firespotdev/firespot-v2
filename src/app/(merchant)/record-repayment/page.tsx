...
function RecordRepaymentContent() {
  ...
  const isLoadingDebt = useMemo(() => {
    ...
    if (
      resolvedCustomerId &&
      (isLoadingCustomerSales || isFetchingCustomerSales)
    )
      return true
    return false
  }, [
  ...
  const customerName = useMemo(() => {
    ...
    if (
      typeof firstSale?.customerId === 'object' &&
      firstSale?.customerId?.name
    ) {
      return firstSale.customerId.name
    }
    if (
      typeof firstSale?.customerId === 'object' &&
      firstSale?.customerId?.businessName
    ) {
    ...
    if (
      firstSale?.description &&
      firstSale.description.startsWith('Sale for ')
    ) {
  ...
  const handleOpenSummaryDrawer = () => {
    ...
    openDrawer({
      ...
      props: {
        sale,
        outstandingSales:
          customerOutstandingSales.length > 0
  ...
  if (isLoadingDebt) {
    return (
      <div className="min-h-dvh flex items-center justify-center bg-white">
        <LoaderCircle innerBg="white" />
      </div>
    )
  }

  return (
    <div className="h-dvh bg-white flex flex-col font-satoshi justify-between max-w-md mx-auto relative overflow-hidden">
      <div className="flex-1 flex flex-col min-h-0">
        <header className="px-4 py-3 flex justify-between items-center relative w-full border-b border-gray-100 shrink-0">
          <CircularIconButton
            icon="arrow-left"
            size="md"
            onClick={() => router.back()}
          ...
          {isLoadingDebt ? (
            ...
            <>
              ...
              <div className="flex items-center justify-center font-bold text-[42px] tracking-tight text-black">
                ...
                <span className="w-0.5 h-10 bg-[#0085FF] ml-1 animate-caret-blink" />
              </div>
            </>
          )}
        </div>
      ...
      <div className="px-4 py-3.5 border-t border-b border-[#F1F1F1] bg-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#0085FF] flex items-center justify-center shrink-0">
            <Wallet size={20} />
          </div>
          <div>
        ...
        <button
          ...
          className="px-4 py-2 bg-[#F4F6F8] hover:bg-gray-200 active:bg-gray-300 text-[11px] font-bold text-black rounded-full transition-colors uppercase cursor-pointer"
        >
          CHANGE
        </button>
      </div>
