...
interface RailPickerDrawerProps {
  savedCardPaymentsEnabled?: boolean
...
export function RailPickerDrawer({
  savedCardPaymentsEnabled = false,
  ...
  const hasMultiplePaymentOptions = Boolean(paystackChannels?.length)
  ...
  const handleSelect = (rail: PaymentRail) => {
    ...
    if (rail === 'multiple' && !hasMultiplePaymentOptions) return
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-3">
        ...
        <div
          className={`${savedCardPaymentsEnabled ? 'mt-3 ' : ''}overflow-hidden rounded-[12px] border border-[#F1F1F1] bg-white shadow-[0px_4px_8px_0px_rgba(0,0,0,0.04)]`}
        >
          ...
          <button
            ...
            className={`flex w-full items-center justify-between p-3 text-left transition-colors ${
              hasMultiplePaymentOptions ? 'border-t border-[#E3E5E8]' : ''
            }`}
          >
            <div className="flex min-w-0 items-center gap-4">
